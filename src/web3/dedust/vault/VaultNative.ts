...
import { Asset, ReadinessStatus } from "@/web3/dedust/common";
import { PoolType } from "@/web3/dedust/pool";

export type SwapToJettonParams = {
...
export class VaultNative extends Vault {
    ...
    async sendSwap(provider: ContractProvider, via: Sender, params: SwapToJettonParams, gasAmount?: bigint) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: VaultNative.swapMessage(params),
            value: params.amount + (gasAmount ?? toNano("0.2"))
        });
    }

    static swapMessage({ queryId, amount, poolAddress, limit, swapParams, next }: SwapToJettonParams) {
        return beginCell()
